...
def generate_hnl_events(m, U, events_to_inject=100000, experiment="OGTPC1"):
    ...
    if os.path.isfile(f"output_prob_2E-6/{path}"): 
    ...
    os.makedirs("output_prob_2E-6", exist_ok=True)
    controller.SaveEvents(f"output_prob_2E-6/{experiment}_Dipole_M{m:.2e}_Umu{U:.2e}_example", fill_tables_at_exit=True, save_int_probs=True)
