...
def generate_hnl_events(m, U, events_to_inject=100000, experiment="OGTPC2"):
    
    path="OGTPC2_Dipole_M%2.2e_Umu%2.2e_example.parquet"%(m,U)
    ...
    table_dir = os.path.join(xs_path, f"Dipole_M{m:.2e}_Umu4{U:.2e}")
...
def main():
    ...
    for m, u in zip(m_sample[0:20], U_sample[0:20]):
