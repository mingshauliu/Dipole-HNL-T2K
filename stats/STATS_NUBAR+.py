...
class Constants:
    ...
    DATA_DIR = Path('../MC/output_nubar_NEW')
    SIGNAL_DIR = Path('./STATS_NUBAR+_ERRu.csv')
    ERROR_BOOL = True
...
def create_error_interpolator(
    filename: str = "numubar_flux_error.txt",
    skip_rows: int = 0,
    m_n_min: float = 10,
    m_n_max: float = 2000,
    m_n_step: float = 25
...
def load_data(exp: str, m: float, mu: float, POT: dict) -> Optional[ParticleData]:
    ...
    try:
        ...
        weights = ak.to_numpy(data['event_weight']) * fid_flag * POT['ND280+']
    ...
    if particle_data is None or np.sum(particle_data.weights) == 0:
        ...
        if len(weights) == 0:
            ...
            if error_flag:
                ...
                print('yea thats right lets go bruther')
