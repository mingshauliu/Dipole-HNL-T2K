...
OGTPC_boxes = np.array([
    [0, 0, -0.375, 1.7, 1.96, 0.563],
    [0, 0, 0.99, 1.7, 1.96, 0.563],
    [0, 0, 2.355, 1.7, 1.96, 0.563]
]) # TPC dimensions in ND280

TPC_boxes = np.array([
    [0, 0, 1.035, 1.7, 1.96, 0.563],
    [0, 0, 2.4, 1.7, 1.96, 0.563],
    [0, 0, 3.765, 1.7, 1.96, 0.563]
]) # TPC dimensions in ND280+

HATPC_boxes = np.array([
    [0, 0.74, -0.795, 1.592, 0.27, 1.709],
    [0, -0.74, -0.795, 1.592, 0.27, 1.709]
]) # HA-TPC dimensions in ND280+
...
def load_data(exp, m, mu, POT):
    ...
    try:
        if os.path.isfile('../output_nubar_NewVol_V2/' + path):
            data = ak.from_parquet("../output_nubar_NewVol_V2/" + path)
            
            mask = data['vertex'][:,0,2]<-10
            dec_flag = data["primary_type"] == 5914
            fid_flag = data["in_fiducial"][dec_flag]*mask # kill da bump
            weights = np.array(np.squeeze(data['event_weight'] * fid_flag * (POT['ND280NU'])))
...
def write_signal_strength(exp, i, m, mu, signal_strength, filename="nobump_nubar.csv"):
