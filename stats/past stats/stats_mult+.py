...
def load_data(exp, m, mu, POT):
    ...
    if os.path.isfile('../TPC/output/' + path):
        data = ak.from_parquet("../TPC/output/" + path)
        
        dec_flag = data["primary_type"] == 5914
        fid_flag = data["in_fiducial"][dec_flag]
        weights = np.array(np.squeeze(data['event_weight'] * fid_flag * (POT['ND280+'])))
...
def write_signal_strength(exp, i, m, mu, signal_strength, filename="signal_strengths_nd280+.csv"):
...
def main():
    ## Constants
    POT = {'ND280+':2.6098758621e22, 'ND280NU':1.234e21, 'ND280NUBAR':6.29e20}
    ...
    experiments = ['ND280UPGRD','TPC2', 'TPC3', 'TPC4','TPC5']  # Add more experiments as needed
    ms = m_sample[index]
    mus = mu_sample[index]

    # Prepare arguments for multiprocessing
    args_list = [(exp, i, m, mu, POT) for exp in experiments for i, m, mu in zip(index, ms, mus)]
