...
def load_data(exp, m, mu, POT):
    ...
    if os.path.isfile('../output_final_bar/' + path):
        data = ak.from_parquet("../output_final_bar/" + path)
        
        sim_pot = POT['ND280NU'] if 'OG' in exp else POT['ND280+']
        # Select events in FGD fiducial volumes
        weights = np.array(np.array(data['event_weight'] * is_points_in_boxes(data['vertex'][:,1], FGD_boxes)) * sim_pot)
        mask = np.logical_not(np.isnan(weights))
        
        weights = weights[mask]

        nuout_momenta = process_particle_data(data, "secondary_types", 5910, "secondary_momenta")[mask,:]
        N_momenta = np.squeeze(data["primary_momentum"][data["primary_type"] == 5914])[mask,:]
        gamma_momenta = process_particle_data(data, "secondary_types", 22, "secondary_momenta")[mask,:]
...
def generate_samples(gamma_ETotal, weights, nuout_momenta, N_momenta, gamma_direction, m, mu,
    ...
    total_samples = int(np.sum(weights[np.logical_not(np.isnan(weights))]) * samples_per_etotal)
...
def write_signal_strength(exp, i, m, mu, signal_strength, filename="signal_fgd_bar.csv"):
...
def main():
    ...
    experiments = ['OGTPC1', 'OGTPC2', 'OGTPC3','ND280UPGRD','TPC2', 'TPC3', 'TPC4','TPC5']
    
    filename="signal_fgd_bar.csv"
